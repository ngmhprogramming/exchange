...
int main() {
    ...
    book.addOrder(buy1, res); // Execute a trade for 100
    std::cout << book;
    for (const auto &report : res) {
        std::cout << "Trade Executed " << report.quantity << " @ "
                  << report.price << " (Maker ID: " << report.maker_id
                  << ", Taker ID: " << report.taker_id << ")\n";
    }
    ...
    book.addOrder(buy2, res); // Execute a trade for 40, leaving 60
    std::cout << book;
    for (const auto &report : res) {
        std::cout << "Trade Executed " << report.quantity << " @ "
                  << report.price << " (Maker ID: " << report.maker_id
    ...
    book.addOrder(buy3, res); // No trade should be executed
    std::cout << book;
    for (const auto &report : res) {
        std::cout << "Trade Executed " << report.quantity << " @ "
                  << report.price << " (Maker ID: " << report.maker_id
